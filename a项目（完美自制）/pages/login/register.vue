	...
	<view class="register">
		<view class="content">
			...
			<view class="main">
				...
				<wInput v-model="verCode" type="text" maxlength="4" placeholder="验证码" value="verCode" isShowCode ref="runCode" @setCode="getVerCode()"></wInput>
			</view>
...
<script>
var _this;
import wInput from '../../components/login_Plus/watch-login/watch-input.vue'; //input
import wButton from '../../components/login_Plus/watch-login/watch-button.vue'; //button
...
export default {
	...
	methods: {
		/*始终做不到把request做成同步请求,只能用下三滥的方法了*/
		getData(surl, str,p) {			
				uni.request({
					url: surl,
					method: 'GET',
		...
		getVerCode() {
			...
			this.getData(this.$api.user.getCode,"getCode",p)
		},
		startReg() {
			//注册
			if (this.isRotate) {
			...
			/*发送数据到数据库*/
			// var url = 'http://127.0.0.10:1080/Home/Login/register?phone=' + _this.phoneData + '&password=' + _this.passData + '&Code04=' + _this.verCode;
			// this.getData(url, 'startReg');
			var url=this.$api.user.register;
			let a={
				"phone":_this.phoneData,
				"password":_this.passData,
				"code04":_this.verCode
			};
			this.postData(url,'startReg',a);
		},
		requestafter(str) {
			console.log(this.data);
			if (str == "startReg") {
				if(this.data.data.phone!=null&&this.data.data.phone!="")
				...
				setTimeout(function() {
					uni.navigateTo({
						url: '/pages/login/login?phone='+encodeURIComponent(JSON.stringify(phone))
					});
				}, 2000);
				}
				else{
			...
			if(str=="getCode"){
				console.log('获取验证码');
				if (_this.data.data.code == '2000') {
					/*验证码次数减1*/
					if (_this.data.data.code04 != '' && _this.data.data.code04 != null) {
						// alert('验证码(次数-1):'+res.data.code04);
						console.log('验证码' + _this.data.data.code04);
						_this.verCode = _this.data.data.code04;
					}
					uni.showToast({
						icon: 'none',
						position: 'bottom',
				...
				if (_this.data.data.code == '200') {
					/*成功获得第一次验证码*/
					if (_this.data.data.code04 != '' && _this.data.data.code04 != null) {
						// alert('验证码'+res.data.code04);
						_this.verCode = _this.data.data.code04;
						console.log('验证码:' + _this.data.data.code04);
						uni.showToast({
							icon: 'none',
				...
				_this.respongs = _this.data.data;
			}
...
<style>
@import url('../../components/login_Plus/watch-login/css/icon.css');
